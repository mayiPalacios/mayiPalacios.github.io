     ...
     switch(e.target.name){
             ...
             const salary =Number.parseFloat(e.target.value);
              if(salary <= 0 ){
               ...
               document.getElementById('inp-salary').classList.add('input-numer-error');
            
              }else{
                document.getElementById('inp-salary').classList.remove('input-numer-error');
             ...
             case "years":
                ...
                if(year < 0 || year>35){
                 ...
                 document.getElementById('inp-years').classList.add('input-numer-error');
                  ...
                  document.getElementById('inp-years').classList.remove('input-numer-error');
             ...
             case "days":
                ...
                if(days <= 0 || days>365){
                 ...
                 document.getElementById('inp-days').classList.add('input-numer-error');
                  ...
                  document.getElementById('inp-days').classList.remove('input-numer-error');
...
document.getElementById('btn-aguinaldo').addEventListener('click',function(){
    ...
    if(inpsalary.value == ""){
    document.querySelector('#groupSalary .alert-empty ').classList.add('quitAler-empty');
    }else{
        document.querySelector('#groupSalary .alert-empty ').classList.remove('quitAler-empty');
    }
    if(inpYear.value == ""){
        document.querySelector('#group-years .alert-empty ').classList.add('quitAler-empty')
    }else{
        document.querySelector('#group-years .alert-empty ').classList.remove('quitAler-empty')
    }
    if(inpDays.value == ""){
        document.querySelector('#group-days .alert-empty ').classList.add('quitAler-empty');
    }else{
        document.querySelector('#group-days .alert-empty ').classList.remove('quitAler-empty');
    }
    ...
    if(nSalary != 0 && nYear>=0 && nYear<35 && nDay>0 && nDay<365){
        ...
        if(nYear<1){
           ...
           const aguinaldo1 = (nDay*dias)/365;
           
            document.getElementById("ven-agui").style.display = "block";
         
        }
        if(nYear>=1 && nYear<=3 ){
            const  aguinaldo2 = (nSalary*15)/30;
          ...
          if(nYear>3 && nDay>1 && nYear<=10){
            const   aguinaldo3 = (nSalary*19)/30;
        ...
        if(nYear>10 && nDay>1){
            const  aguinaldo4 = (nSalary*21)/30
               var dshow = document.getElementById("aguinaldoV");
               var p = document.createElement('p');
